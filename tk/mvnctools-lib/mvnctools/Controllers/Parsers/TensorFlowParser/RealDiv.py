...
def load(obj, operations):
    ...
    x = Scale(obj.name, [inp.name], getOutputNames(obj))

    divisor = obj.inputs[1]
    value = findTensorValue(divisor.name, operations)

    beta_zero = np.zeros(value.shape)
    x.assignMultiplier(1/value)
    x.assignBiasBeta(beta_zero)
