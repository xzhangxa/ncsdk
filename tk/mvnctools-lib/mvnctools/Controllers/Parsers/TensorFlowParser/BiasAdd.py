...
def load(obj, operations):
    ...
    # first input is always assumed to be bias input
    bias_inputs = [getInputNames(obj)[0]]
    x = Bias(obj.name, bias_inputs, getOutputNames(obj))

    # second input always assumed to be the bias
    bias = findTensorValue(getInputNames(obj)[1], operations)
    x.loadBias(bias)
