...
from mvnctools.Controllers.Parsers.Parser.Conversion import Conversion, getDefaultConversionLayerFormatPool
import mvnctools.Models.Layouts as Layouts

# TODO: For datatype stuff
import numpy as np
...
def findSolution(producerMode, consumer, tensor, ignoreEnclosure=False):
    ...
    if ignoreEnclosure:
        ...
        tensorAxisDemand = [1 if x - y != 0 else 0 for (x, y) in zip(tshape, eshape)]
    ...
    for consumerMode in reducedFormatPool(producerMode, consumer):
        ...
        solutionExists = producerOutputFmt.compatible(consumerInputFmt, tensor.getShape(), tensorAxisDemand)
...
def getVotesRaw(producerFormatPool, consumers, tensor, skipMarkedConsumers=False, ignoreEnclosure=False):
    """
    ...
    for pModeIdx, pMode in enumerate(producerFormatPool):
        for consumerIdx, consumer in enumerate(consumers):
            ...
            sol = findSolution(pMode, consumer, tensor, ignoreEnclosure=ignoreEnclosure)
            if sol:
                try:
...
def getVotes(producer, consumers, tensor, skipMarkedConsumers=False):
    return getVotesRaw(producer.formatPool, consumers, tensor, skipMarkedConsumers)
...
def fixTensors(parsedLayers, scheduler, myriadX):
    ...
    from mvnctools.Controllers.Parsers.Parser.Hw import HwConvolution
    from mvnctools.Controllers.Parsers.Parser.Hw import HwPooling
    from mvnctools.Controllers.Parsers.Parser.Hw import HwFC
    ...
    from mvnctools.Controllers.Parsers.Parser.Convolution2D import ConvolutionDepthWise2D

    # Set the tensor formats for each layer
    # Notice that hardware layers only output in interleaved mode (simplification)
    """
        If HwConv or HwFC, set the format for scheduling
    """
    for layer in parsedLayers:
        if isinstance(layer, HwConvolution) or isinstance(layer, HwFC) or isinstance(layer, HwPooling):
            # The hardware compiler can potentially pad the input of a hw convolution, in order
            # to improve performance.
            inputChannels, _, _ = scheduler.solution[layer.getName().stringifyName()]
            layer.adjustFormatPoolForSchedule(inputChannels)

        if not hasattr(layer, 'formatPool'):
            raise Exception('Layer {} does not specify supported formats'.format(layer)) # If there is an error here, a software layer may not have had its formatPool set.
        ...
        if layer_type == 'BLOB':
            ...
            for producerFormatPoolIdx, consumersVotedForThisFormat in votes.items():
                if len(consumersVotedForThisFormat) == len(consumers):
                    ...
                    # Compare the tensor to the encloser, and make a mask of where stride is needed.
                    tshape = tensor.getShape()
                    eshape = tensor.getTopEncloserRecursive().getShape()
                    tensorAxisDemand = [1 if x - y != 0 else 0 for (x, y) in zip(tshape, eshape)]

                    # Readjust the formatPool of the consumers to reflect the choice taken
                    for consumerIdx, (_, consumerInputFmt) in consumersVotedForThisFormat:
                        consumers[consumerIdx].formatPool = matchConsumingFormat(consumers[consumerIdx].formatPool, consumerInputFmt, tensor.getShape(), tensorAxisDemand)
                    break
    ...
    # Optimistically set the layout for tensors that are fed into layers with multiple input tensors.
    for name in nx.lexicographical_topological_sort(g):
        layer_type = g.node[name]['type']
            ...
            for fmt in originalFormatPool:
                ...
                if counter == len(layer.getInputTensors()):
                    ...
                    for tensor in layer.getInputTensors():
                        ...
                        # Optimistically set the tensor for layers that reach a consensus
                        votes = getVotes(producer, consumers, tensor)
    ...
    for name in nx.lexicographical_topological_sort(g):
        ...
        if layer_type == 'BLOB':
            ...
            if not tensor.format:
                ...
                # Set the tensor in this mode and insert conversion layers for the rest.
                producerFormatPoolIdx, consumersIdxVotedForThisFormat = getPrevelantChoice(votes)
                ...
                if producerFormatPoolIdx:
                    ...
                    for consumerIdx, (_, consumerInputFmt) in votes[producerFormatPoolIdx]:
                        consumers[consumerIdx].formatPool = matchConsumingFormat(consumers[consumerIdx].formatPool, consumerInputFmt, tensor.getShape(), tensorAxisDemand)
                ...
                votes = getVotesRaw(conversionLayerFmtPool, consumers, tensor, skipMarkedConsumers=True, ignoreEnclosure=True)


                for mode, layers in minimizeConversionLayers(OrderedDict(votes)):
                    # Add conversion layers (and tensors) between tensor and consumers.

                    # Create the Conversion layer and the tensor
                    convertedTensorName = '{}_converted'.format(tensor.getName().stringifyOriginalName())
                    convertedTensor = UnpopulatedTensor(shape = tensor.getShape())
                    convertedTensor.setName(MangledName(OriginalName(convertedTensorName)))
                    ...
                    convertLayerName = "{}_convert".format(producer.getName().stringifyOriginalName())
                    convertLayer = Conversion(convertLayerName, [tensor.getName()], [convertedTensor.getName()])
                    ...
                    convertLayer.loadOutputTensorSizes([convertedTensor.getShape()])

                    convertLayer.deriveFormatPool(g.node[name]['ref'].format[1], convertedTensor.format[1])
                    ...
                    g.add_node(convertLayer.getStringifiedName(), type="OP", ref=convertLayer)
                    g.add_node(convertedTensor.getName().stringifyName(), type="BLOB", ref=convertedTensor)
                    g.add_edge(name, convertLayer.getStringifiedName())
                    g.add_edge(convertLayer.getStringifiedName(), convertedTensor.getName().stringifyName())
                    ...
                    for nodeName in list(g.successors(name)):
                        if g.node[nodeName]['ref'] in layerRefs:
                            ...
                            g.add_edge(convertedTensor.getName().stringifyName(), nodeName)

                            # TODO: Update the tensors that corresponds to these nodes
                            g.node[nodeName]['ref'].inputTensorNames =  [convertedTensor.getName()]
                            g.node[nodeName]['ref'].setInputTensors([convertedTensor])
                            g.node[nodeName]['ref'].loadInputTensorSizes([convertedTensor.getShape()])
        ...
        if layer_type == 'BLOB':
            ...
            if not tensor.format:
                ...
                for consumer in consumers:
                    ...
                    # TODO: Dont have None in conversionLayerFmtPool
                    convertedTensor.format = (producer.formatPool[0][1], consumer.formatPool[0][0])
                    ...
                    g.add_edge(name, convertLayer.getStringifiedName())
                    g.add_edge(convertLayer.getStringifiedName(), convertedTensor.getName().stringifyName())
                    g.add_edge(convertedTensor.getName().stringifyName(), consumer.getStringifiedName())
                    ...
                    # Find the index where this nodes has tensor whose name is `name`
                    idx = -1
                    ...
                    consumer.inputTensorNames =  [t.getName() for t in inputTensors]
                    ...
                    print('Adding edge between', name, 'and', convertLayer.getStringifiedName())
                    print('Adding edge between', convertLayer.getStringifiedName(), 'and', convertedTensor.getName().stringifyName())
                    print('Successors before mutation', successors_before_mutation)
    ...
    for name in nx.lexicographical_topological_sort(g):
        if g.node[name]['type'] == 'BLOB':
            ...
            # print(producers[0].formatPool[0][1].compatible(consumers[0].formatPool[0][0], tensor.getShape()))
            # print(producers[0].formatPool[0][1].ensureCompatibility(consumers[0].formatPool[0][0], tensor.getShape()))
            newShape = producers[0].formatPool[0][1].ensureCompatibility(consumers[0].formatPool[0][0], tensor.getShape())

            if newShape != tensor.getShape():
                newTensor = UnpopulatedTensor(shape=newShape)
                print('\tPlace {} ({}) in {}'.format(tensor.getName().stringifyName(),
                    tensor.getShape(), newTensor.getShape()))

                newTensor.setName(MangledName(OriginalName("satisfier")))
                newTensor.setLayout(tensor.getLayout())
                newTensor.setDatatype(np.float16)
    ...
    # ii) More importantly, layers that accept any layout (i.e. any is a special format type).
