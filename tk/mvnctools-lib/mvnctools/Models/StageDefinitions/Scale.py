...
class Scale(Op):
    ...
    def adapt_fields(self, emulator, layer):
        ...
        t = PopulatedTensor(np.array(layer.getMultiplier()).astype(np.float16))
        t.setLayout((0, 1, 2, 3))
        t.setDatatype(np.float16)
        emulator.tapsBUF = BufferEmulator(t.resolve())

        b = PopulatedTensor(np.array(layer.getBiasBeta()).astype(np.float16))
        b.setLayout((0, 1, 2, 3))
